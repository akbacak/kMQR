...
queryIndex = xlsread('/home/ubuntu/Desktop/Thesis_Follow_Up_2/dmqRetrieval/lamdaDataset/qLabels_4d.xls');   % Reads randomly choosen query pairs from excell file
 % Reads randomly choosen query pairs from excell file
...
queryIndex4 = queryIndex(4,:); 
 ...
 for l = 1:240 % Number of Query Pairs , CAN TRY FOR DIFFERENT HAMMING RADIUS ALSO ?????? how?
   ...
   predicted_labels{l,:} = targets(Retrieved_Items_Ranked{l,:} , :);
    ...
    union_of_query_labels_MQR{l,:} = or(union_of_query_labels_MQR{l,:}, targets(queryIndex(4,l), : ));
   ...
   %%%%%%%%%%%%%%%%%%%% Precision & Recall for DMQIR  %%%%%%%%%%%%%%%%%%%%
    ...
    for j= 1:maxFront
        predicted_labels_DMQIR{l,j} =      targets(pf_idx{l,j}(:,5),:);
        ...
        union_of_query_labels_DMQIR{l,:} = or(union_of_query_labels_DMQIR{l,:}, targets(queryIndex(4,l), : ));
