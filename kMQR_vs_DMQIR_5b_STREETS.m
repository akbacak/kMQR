...
queryIndex5 = queryIndex(5,:); 
 ...
 for l = 1:240 % Number of Query Pairs , CAN TRY FOR DIFFERENT HAMMING RADIUS ALSO ?????? how?
   ...
   predicted_labels{l,:} = targets(Retrieved_Items_Ranked{l,:} , :);
    ...
    union_of_query_labels_MQR{l,:} = or(union_of_query_labels_MQR{l,:}, targets(queryIndex(5,l), : ));
   ...
   %%%%%%%%%%%%%%%%%%%% Precision & Recall for DMQIR  %%%%%%%%%%%%%%%%%%%%
    ...
    for j= 1:maxFront
        predicted_labels_DMQIR{l,j} =      targets(pf_idx{l,j}(:,6),:);
        ...
        union_of_query_labels_DMQIR{l,:} = or(union_of_query_labels_DMQIR{l,:}, targets(queryIndex(5,l), : ));
